# 🚀 迭代更新笔记 (Update Notes)
- **✨ 核心功能升级**：新增近 7 日历史趋势展示、sparkline 走势摘要、学生资金分层、风险画像、热门理财选项地图、理财思维训练、定投模拟器，以及零构建静态前端工作台。前端现在支持 `web/data.json` schema 校验、数据过期提醒和默认示例数据兜底。
...
- **🐛 细节与健壮性优化**：补充 BaoStock / akshare 获取失败时的兜底处理，历史文件损坏时自动备份，JSON 导出前强制校验数据契约，前端在数据缺失、版本不兼容或数据超过 3 天未更新时给出明确提示。测试覆盖估值边界、历史趋势、风险画像、资金分层、JSON 导出和前端核心逻辑。
