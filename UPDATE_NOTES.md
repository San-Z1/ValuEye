# 🚀 迭代更新笔记 (Update Notes)
- **✨ 核心功能升级**：新增近 7 日历史趋势展示、sparkline 走势摘要、学生资金分层、风险画像、热门理财选项地图、理财思维训练、定投模拟器，以及零构建静态前端工作台。前端现在支持 `web/data.json` schema 校验、数据过期提醒、默认示例数据兜底、无缓存刷新、理财产品地图展示和投资前准备度评分。新增 `python -m finance_monitor.main --demo-data`，可在无网络时生成课堂演示数据。
